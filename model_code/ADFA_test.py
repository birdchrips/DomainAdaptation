...
class ADFA_LSTM(nn.Module):
    def __init__(self, hidden_dimension, word_vecs, n_hidden, n_step, LearningRate, dropout_p=0.2):
        super(ADFA_LSTM, self).__init__()
        self.emb = nn.Embedding.from_pretrained(torch.tensor(word_vecs, dtype=torch.float).cuda(), freeze=True)
    ...
    def print_loss_graph(self):
        plt.plot(self.batch_loss_list, label='loss')
        plt.legend()
...
class ADFA_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        return torch.tensor(out).cuda(), torch.tensor(self.label[i], dtype=torch.long).cuda()
